...
public class FinalDb {
	...
	public boolean saveBindId(Object entity) {
		...
		if (entityKvList != null && entityKvList.size() > 0) {
			...
			debugSql("insert into " + tf.getTableName() + " values " + cv.describeContents());
			if (id == -1)
				return false;
	 ...
	 */
	public void dropDb() {
		Cursor cursor = db.rawQuery("SELECT name FROM sqlite_master WHERE type ='table' AND name != 'sqlite_sequence'",
				null);
		if (cursor != null) {
	...
	private void exeSqlInfo(SqlInfo sqlInfo) {
		if (sqlInfo != null) {
			...
			debugSql(sqlInfo.getSql(), args);
			db.execSQL(sqlInfo.getSql(), args);
		} else {
	 ...
	 *            排序字段
	 */
	public <T> List<T> findAllByWhereWithOrder(Class<T> clazz, String strWhere, String orderBy) {
		checkTableExist(clazz);
	 ...
	 *            排序字段
	 */
	public <T> List<T> findAllByWhereWithOrder(Class<T> clazz, String strWhere, String orderBy, String... args) {
		checkTableExist(clazz);
		return findAllBySql(clazz, SqlBuilder.getSelectSQLByWhere(clazz, strWhere) + " ORDER BY " + orderBy, args);
	}
	...
	private void debugSql(String sql, Object... args) {
		...
		android.util.Log.d("Debug-SQL", ">>>>>>  " + sql + " args : " + sb.toString());
	}
